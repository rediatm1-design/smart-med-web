    ...
    <div>
      ...
      <div className="section">
        ...
        >
         ...
         <p>Form state: {showForm ? "OPEN" : "CLOSED"}</p>
        {showForm ? (
  <div
    className="form-card"
    style={{
...
function Medications({
  ...
  const fetchSchedules = async (patientId) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/patients/${patientId}/medication-schedules`,
        {
          headers: {
            Accept: "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      ...
      await fetchSchedules(selectedPatientId);
  ...
  const handleRecordDose = async (scheduleId, status) => {
    if (!selectedPatientId || !scheduleId) {
      alert("Patient and medication schedule are required.");
      return;
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/dose-events`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
      ...
      if (!response.ok) {
        alert(data.message || "Failed to record dose event.");
        return;
      }

      alert(`Dose marked as ${status}.`);
    } catch (err) {
      alert("Unable to connect to server.");
    }
  };
